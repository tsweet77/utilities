//Compile: g++ -O3 -Wall -static Multi-Hash_OpenSSL_Preserve_Original.cpp -o Multi-Hash_OpenSSL_Preserve_Original.exe -lssl -lcrypto -lcrypt32 -ladvapi32 -lws2_32 -I./openssl/include
...
int main() {
    std::cout << "Multi-Hashing SHA2 256 Preserve Original by Anthro Teacher." << std::endl;
    ...
    if (isFilename(original)) {
        ...
        while (std::getline(file, line)) {
            ...
            for (long level = 1; level <= num_hash_levels; ++level) {
                ...
                hashed_text = sha256_hash(repeated_hash + ": " + original_text);
                std::transform(hashed_text.begin(), hashed_text.end(), hashed_text.begin(), ::toupper);
            }
        ...
        for (int level = 1; level <= num_hash_levels; ++level) {
            ...
            hashed_text = sha256_hash(repeated_hash + ": " + original);
            repeated_hash = "";
            std::transform(hashed_text.begin(), hashed_text.end(), hashed_text.begin(), ::toupper);
        }
